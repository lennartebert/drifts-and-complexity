...
def population_distribution() -> PopulationDistributions:
    ...
    class MockPopulationDistributions:
        def __init__(self):
            ...
            self.dfg_edges = MockCount(12)      # 12 distinct DFG edges (same as transitions)
...
def assert_close(a: float, b: float, tolerance_type: str = 'default', rtol: float = None, atol: float = None) -> None:
...
def create_traces_from_patterns(patterns: List[List[str]]) -> List[Trace]:
    ...
    return [create_trace_from_activities(pattern, f"trace_{i}") for i, pattern in enumerate(patterns)]
