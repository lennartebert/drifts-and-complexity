...
class TestNaivePopulationExtractor:
    ...
    def test_apply_empty_window(self):
        ...
        with pytest.raises(
            ValueError, match="Window must contain traces for population extraction"
        ):
    ...
    @patch(
        "utils.population.extractors.naive_population_extractor._counts_trace_variants"
    )
    ...
    def test_apply_population_distributions_structure(self):
        ...
        with (
            patch(
                "utils.population.extractors.naive_population_extractor._counts_activities"
            ) as mock_acts,
            patch(
                "utils.population.extractors.naive_population_extractor._counts_dfg_edges"
            ) as mock_dfg,
            patch(
                "utils.population.extractors.naive_population_extractor._counts_trace_variants"
            ) as mock_vars,
...
class TestCountsActivities:
    ...
    @patch(
        "utils.population.extractors.naive_population_extractor.attributes_get.get_attribute_values"
    )
...
class TestCountsTraceVariants:
    ...
    @patch(
        "utils.population.extractors.naive_population_extractor.variants_filter.get_variants"
    )
...
class TestBuildNaiveDistributionFromCounts:
    ...
    def test_build_naive_distribution_complex_labels(self):
        ...
        counts: Counter = Counter({("A", "B"): 2, ("A", "B", "C"): 1, ("A", "C"): 1})
