...
from tests.conftest import (
    ...
    population_distribution, empty_population_distribution
)
...
class TestLocalMetricsAdapter:
    ...
    def test_available_metrics(self, adapter):
        ...
        for metric_name in ALL_METRIC_NAMES:
            if metric_name not in entropy_metrics:  # Entropy metrics are from vidgof adapter
                assert metric_name in metrics, f"Expected metric {metric_name} not found"
    ...
    def test_compute_measures_with_population_distribution(self, adapter_distribution, standard_test_log, population_distribution):
    ...
    def test_compute_measures_for_windows_batch(self, adapter, simple_traces, complex_traces):
        ...
        windows = [
            Window(id="simple", size=len(simple_traces), traces=simple_traces, population_distributions=None),
            Window(id="complex", size=len(complex_traces), traces=complex_traces, population_distributions=None)
        ]
    ...
    def test_compute_measures_for_windows_with_existing_stores(self, adapter, simple_traces, complex_traces):
    ...
    def test_prefer_parameter_behavior(self, standard_test_log, population_distribution):
