...
from utils.complexity.metrics.registry import available_metric_names, discover_metrics
...
class TestMetricsCalculation:
    ...
    def test_all_metrics_computed(self, computed_metrics):
        ...
        expected_minimum = (
            len(ALL_METRIC_NAMES) - 4
        )  # 4 entropy metrics are typically skipped
        assert (
            computed_count >= expected_minimum
        ), f"Expected at least {expected_minimum} metrics, got {computed_count}"
        ...
        critical_skipped = [
            m for m in skipped if m in ALL_METRIC_NAMES and m not in entropy_metrics
        ]
        assert (
            len(critical_skipped) == 0
        ), f"Critical non-entropy metrics were skipped: {critical_skipped}"
    ...
    def test_metric_values_match_expected(self, expected_values, computed_metrics):
        ...
        for metric_name, expected_value in expected_metrics.items():
            ...
            if isinstance(expected_value, (int, float)) and isinstance(
                ...
                if not np.isclose(
                    expected_value, computed_value, rtol=0, atol=tolerance
                ):
                    mismatches.append(
    ...
    def test_hand_computed_values_verification(self, expected_values, computed_metrics):
        ...
        for metric_name, expected_value in expected_metrics.items():
            ...
            if isinstance(expected_value, (int, float)) and isinstance(
                ...
                assert np.isclose(
                    expected_value, computed_value, rtol=0, atol=1e-10
    ...
    def test_metric_metadata_consistency(self, computed_metrics):
        ...
        for metric_name, data in results.items():
            ...
            ), f"Metric {metric_name} should have metadata"
    ...
    def test_metric_properties(self, computed_metrics):
        ...
        for metric_name in non_negative_metrics:
            if metric_name in results and results[metric_name]["value"] is not None:
                ...
                assert (
                    value >= 0
                ), f"Metric {metric_name} should be non-negative, got {value}"

        # Test range properties
        if (
            "Percentage of Distinct Traces" in results
            and results["Percentage of Distinct Traces"]["value"] is not None
        ):
        ...
        if all(
            metric in results and results[metric]["value"] is not None
            for metric in [
                "Min. Trace Length",
                "Max. Trace Length",
                "Avg. Trace Length",
    ...
    def test_metric_consistency_relationships(self, computed_metrics):
        ...
        if all(
            ...
            assert (
                distinct_activities <= total_events
...
class TestMetricRegistryIntegration:
    ...
    def test_all_metrics_discoverable(self):
        ...
        for expected_metric in expected_categories:
            ...
            ), f"Expected metric {expected_metric} not found in registry"
    ...
    def test_metric_orchestrator_integration(self, standard_test_log):
        ...
        store_strict, info_strict = orchestrator_strict.compute_many_metrics(
            metric_names, window
        )
        ...
        store_non_strict, info_non_strict = (
            orchestrator_non_strict.compute_many_metrics(metric_names, window)
        )

        # Should also compute all metrics in non-strict mode for this simple case
        computed_count = len(
            [name for name in metric_names if store_non_strict.has(name)]
        )
        assert computed_count == len(
            metric_names
