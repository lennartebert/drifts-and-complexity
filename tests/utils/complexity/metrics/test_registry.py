...
class TestMetricRegistry:
    ...
    def test_metric_variants(self):
        ...
        for metric_name in multi_variant_metrics:
            ...
            assert len(classes) == 2, f"Metric {metric_name} should have exactly 2 implementations, found {len(classes)}"
            ...
            distribution_based = [cls for cls in classes if 'distribution_based' in cls.__module__]
            
            # Should have exactly one of each type
            assert len(trace_based) == 1, f"Metric {metric_name} should have exactly 1 trace-based implementation, found {len(trace_based)}"
            assert len(distribution_based) == 1, f"Metric {metric_name} should have exactly 1 distribution-based implementation, found {len(distribution_based)}"
            ...
            assert 'trace_based' in trace_module, f"Trace-based implementation should be in trace_based module, found {trace_module}"
            assert 'distribution_based' in distribution_module, f"Distribution-based implementation should be in distribution_based module, found {distribution_module}"
    ...
    def test_dual_variant_metrics_retrieval(self):
        ...
        for metric_name in dual_variant_metrics:
            ...
            classes = get_metric_classes(metric_name)
            assert len(classes) == 2, f"Metric {metric_name} should have exactly 2 variants"
            
            # Separate trace-based and distribution-based classes
            trace_class = next(cls for cls in classes if 'trace_based' in cls.__module__)
            distribution_class = next(cls for cls in classes if 'distribution_based' in cls.__module__)
            ...
            assert trace_class.requires == distribution_class.requires, \
            ...
            assert trace_class != distribution_class, \
    ...
    def test_registry_consistency(self):
        ...
        # Should be the same
        assert names1 == names2, "Registry should be consistent across multiple discovery calls"
        ...
        for name in names1:
            ...
            assert len(classes1) == len(classes2), f"Class count should be consistent for {name}"
...
class TestMetricOrchestratorIntegration:
    ...
    def test_orchestrator_uses_registry(self, standard_test_log):
        ...
        assert len(info.get('skipped', [])) == 0, f"Some metrics were skipped: {info.get('skipped', [])}"
    ...
    def test_orchestrator_variant_selection(self, standard_test_log):
        ...
        store_trace, _ = orchestrator_trace.compute_many_metrics(["Number of Distinct Traces"], window)
        ...
        if store_trace.has("Number of Distinct Traces"):
            ...
            assert measure.meta.get("basis") == "traces", "Should use trace-based implementation"
    ...
    def test_orchestrator_dependency_resolution(self, standard_test_log):
        ...
        assert store.has("Min. Trace Length"), "Min. Trace Length dependency should be resolved"
        assert store.has("Max. Trace Length"), "Max. Trace Length dependency should be resolved"
