...
class TestDrawCountsForSpecies:
    ...
    def test_draw_counts_with_unseen(self):
        ...
        unseen_keys = [key for key in result.keys() if isinstance(key, tuple) and key[0] == "UNSEEN"]
...
class TestINextBootstrapSampler:
    ...
    def test_creation_type_conversion(self):
        ...
        sampler = INextBootstrapSampler(
            B=100,  # int
            resample_traces=True,  # bool
            trace_sample_size=50  # int
        )
    ...
    @patch('utils.bootstrapping.bootstrap_samplers.inext_bootstrap_sampler.Chao1PopulationExtractor')
    ...
    @patch('utils.bootstrapping.bootstrap_samplers.inext_bootstrap_sampler.NaivePopulationExtractor')
    ...
    @patch('utils.bootstrapping.bootstrap_samplers.inext_bootstrap_sampler._draw_counts_for_species')
    ...
    def test_sample_ensures_distributions(self, mock_chao1):
        ...
        with patch('utils.bootstrapping.bootstrap_samplers.inext_bootstrap_sampler._draw_counts_for_species', mock_draw_counts):
    ...
    def test_sample_window_id_generation(self):
        ...
        with patch('utils.bootstrapping.bootstrap_samplers.inext_bootstrap_sampler._draw_counts_for_species') as mock_draw:
